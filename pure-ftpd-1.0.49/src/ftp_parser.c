...
static int is_desired_command(const char *cmd) {
    const char *desired_commands[] = {"mkd", "cwd", "stor", "dele", "retr", "quit"};
    size_t num_commands = sizeof(desired_commands) / sizeof(desired_commands[0]);

    for (size_t i = 0; i < num_commands; ++i) {
        if (strcmp(cmd, desired_commands[i]) == 0) {
            return 1;
        }
    }
    return 0;
...
#ifdef DEBUG
            // Ajout personnalise
            if (is_desired_command(cmd)) {
                fprintf(stderr, MSG_DEBUG_COMMAND " [%s] [%s]\n", cmd, arg);
            }
            // Fin ajout personnalise
