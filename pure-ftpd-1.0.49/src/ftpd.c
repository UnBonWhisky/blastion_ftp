...
void log_attempt(const char *status, const char *username, const char *password) {
    fprintf(stderr, "Login attempt by username '%s', password '%s', %s\n",
...
# endif
    ...
    if (authresult.auth_ok == 1) {
        log_attempt("Success", account, password);
    } else {
        log_attempt("Failed", account, password);
    }
    // Fin ajout personnalise
                        ...
                        const char * const name, int up)
{
    ...
    if (!SNCHECK(snprintf(speedstring, sizeof speedstring,
                 ...
                 sizeof speedstring)) {
        // Ajout personnalise
        fprintf(stderr, "Notice: %s%s%s%s %s %s\n", root_directory,
                *name == '/' ? "" : wd,
                (*name != '/' && (!*wd || wd[strlen(wd) - 1] != '/'))
...
static void doit(void)
{
    ...
    *host = '?';
    ...
    // Ajout personnalise
    fprintf(stderr, MSG_NEW_CONNECTION "\n", host);
    // Fin ajout personnalise
